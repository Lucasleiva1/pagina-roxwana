...
export function SettingsForm({ settings }: { settings: SiteSettings }) {
  return (
    <form action={updateSiteSettingsAction} className="grid gap-5 border border-bone/12 bg-charcoal p-5">
      ...
      <label className="grid gap-2 text-xs font-bold uppercase tracking-rox text-steel">
        Email de contacto
        <input name="contact_email" type="email" defaultValue={settings.contactEmail || ""} className="min-h-11 border border-bone/12 bg-ink px-4 text-sm normal-case tracking-normal text-bone outline-none focus:border-roxgold" />
      </label>
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-xs font-bold uppercase tracking-rox text-steel">
          CTA global
          <input name="global_cta_label" defaultValue={settings.globalCtaLabel || ""} className="min-h-11 border border-bone/12 bg-ink px-4 text-sm normal-case tracking-normal text-bone outline-none focus:border-roxgold" />
        </label>
        <label className="grid gap-2 text-xs font-bold uppercase tracking-rox text-steel">
          URL CTA global
          <input name="global_cta_url" defaultValue={settings.globalCtaUrl || ""} className="min-h-11 border border-bone/12 bg-ink px-4 text-sm normal-case tracking-normal text-bone outline-none focus:border-roxgold" />
        </label>
      </div>

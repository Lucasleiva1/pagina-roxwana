...
import { addOrderNoteAction, updateOrderStatusAction } from "@/lib/command/orderActions";
import { getCommandOrder } from "@/lib/command/orders";
...
export default async function CommandOrderPage({ params }: CommandOrderPageProps) {
  const { id } = await params;
  const detail = await getCommandOrder(id);
  ...
  return (
    <div className="grid gap-8">
      <CommandHeader eyebrow="Pedido" title={detail.order.customer_name_snapshot} description={detail.order.id} />
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="Items" title="PRODUCTOS" />
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="Entrega" title="DIRECCION" />
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="WhatsApp" title="MENSAJE" />
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="Notas" title="EVENTOS" />

...
export default async function CommandCartsPage() {
  const carts = await getCommandActiveCarts();

  return (
    <div className="grid gap-8">
      <CommandHeader eyebrow="Carritos" title="CARRITOS ACTIVOS" description="Clientes con productos guardados antes de convertir a pedido." />
          ...
          carts.map((cart) => (
            <article key={cart.id} className="grid gap-4 border border-bone/12 bg-charcoal p-4 lg:grid-cols-[1fr_auto]">
              ...
              <div className="grid content-center gap-3 text-right">
                ...
                {cart.profile ? (
                  <Link href={`/command/clientes/${cart.profile.id}`} className="border border-bone/20 px-4 py-3 text-xs font-bold uppercase tracking-rox text-bone transition hover:border-roxgold">
                    Ver cliente
                  </Link>
                ) : null}
              </div>

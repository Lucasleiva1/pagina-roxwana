...
import { getCommandCustomer } from "@/lib/command/customers";

type CommandCustomerPageProps = {
  params: Promise<{
    id: string;
  }>;
};

export default async function CommandCustomerPage({ params }: CommandCustomerPageProps) {
  const { id } = await params;
  const detail = await getCommandCustomer(id);
  ...
  return (
    <div className="grid gap-8">
      <CommandHeader eyebrow="Cliente" title={detail.profile.name || "CLIENTE ROXWANA"} description={detail.profile.email || detail.profile.user_id} />
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="Historial" title="PEDIDOS" />
        {detail.orders.length > 0 ? (
          <div className="grid gap-3">
            {detail.orders.map((order) => (
              <Link key={order.id} href={`/command/pedidos/${order.id}`} className="grid gap-3 border border-bone/12 bg-charcoal p-4 transition hover:border-roxgold/50 md:grid-cols-[1fr_auto]">
                <div>
      ...
      <section className="grid gap-4">
        <CommandHeader eyebrow="Carritos" title="CARRITOS" />

...
export default async function AdminCustomersPage() {
  ...
  return (
    <div className="grid gap-8">
      <AdminHeader eyebrow="Clientes" title="CLIENTES" description="Perfiles registrados, historial de pedidos y carritos activos." />
      <div className="grid gap-3">
        {customers.length > 0 ? (
          customers.map((customer) => (
            <Link key={customer.id} href={`/admin/clientes/${customer.id}`} className="grid gap-4 border border-bone/12 bg-charcoal p-4 transition hover:border-roxgold/50 md:grid-cols-[1fr_auto]">
              <div>

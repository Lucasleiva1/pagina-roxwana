...
import { updateOrderStatusAction } from "@/lib/admin/orderActions";
...
export default async function AdminOrdersPage() {
  ...
  return (
    <div className="grid gap-8">
      <AdminHeader eyebrow="Pedidos" title="PEDIDOS" description="Pedidos guardados antes de abrir WhatsApp, con estado y trazabilidad interna." />
          ...
          orders.map((order) => (
            <article key={order.id} className="grid gap-4 border border-bone/12 bg-charcoal p-4 xl:grid-cols-[1fr_auto]">
              <div>
                ...
                <Link href={`/admin/pedidos/${order.id}`} className="headline mt-3 block text-3xl text-bone hover:text-roxgold">
